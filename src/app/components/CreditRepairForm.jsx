...
export default function CreditRepairForm() {
  ...
  // Auto-advance for single choice selections (except "other")
  ...
  const handleBackToForm = () => {
    ...
    setTimeout(() => {
      setIsSuccess(false);
      setStep(5); // Go back to the final step instead of resetting to step 1
      setSwipeState('out');
  ...
  // Helper for dynamic slide classes
  const getStepClasses = (stepNumber) => {
    if (step === stepNumber) {
      return "w-full transition-all duration-500 ease-in-out relative opacity-100 translate-x-0 z-10 pointer-events-auto px-1 pb-4";
    } else if (step > stepNumber) {
      return "w-full transition-all duration-500 ease-in-out absolute top-0 left-0 opacity-0 -translate-x-[100%] z-0 pointer-events-none px-1 pb-4";
    } else {
      return "w-full transition-all duration-500 ease-in-out absolute top-0 left-0 opacity-0 translate-x-[100%] z-0 pointer-events-none px-1 pb-4";
    }
  };
  ...
  return (
    <>
      {/* AGGRESSIVE SCROLLBAR HIDING STYLES */}
      <style dangerouslySetInnerHTML={{__html: `
        /* Hide scrollbar completely everywhere */
        ::-webkit-scrollbar {
      ...
      {isSuccess ? (
        ...
        <div className="flex min-h-screen bg-gray-50 items-start justify-center px-3 sm:px-6 pt-6 sm:pt-12 pb-24 w-full">
          <div className="max-w-4xl w-full relative bg-white p-5 sm:p-10 rounded-2xl sm:rounded-3xl shadow-sm border border-gray-100 space-y-6 animate-fade-in-up">
            ...
            <div className="w-full bg-white rounded-xl shadow-inner border border-gray-200 overflow-hidden h-[80vh]" style={{ minHeight: '500px', maxHeight: '650px' }}>
              <iframe
          ...
          <div className="w-full max-w-2xl bg-white p-5 sm:p-12 rounded-2xl sm:rounded-3xl shadow-xl border border-gray-100 relative flex flex-col transition-all duration-500 ease-in-out">
            ...
            <div className="w-full relative overflow-hidden">
                ...
                <div className={getStepClasses(1)}>
                  ...
                  {/* Animated Expanding Input for "Other" Goal */}
                  <div 
                    className="transition-all duration-500 ease-in-out w-full overflow-hidden min-h-0"
                    style={formData.primaryGoal === 'other' ? { maxHeight: '500px', opacity: 1 } : { maxHeight: '0px', opacity: 0 }}
                  >
                    <div className="w-full flex flex-col pb-2">
                      <label htmlFor="otherGoalReason" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">
                      ...
                      <input
                        ...
                        className="block w-full rounded-xl border-2 border-gray-200 px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base text-gray-900 focus:border-emerald-500 focus:outline-none transition-colors shadow-sm"
                      ...
                      <div className="pt-4 mt-4 sm:mt-6 border-t border-gray-100">
                        <button
                          ...
                          className="w-full flex items-center justify-center space-x-2 bg-gray-900 hover:bg-gray-800 text-white py-3 sm:py-3.5 px-4 rounded-xl text-sm sm:text-base font-semibold transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <span>Continue</span>
                    ...
                    {negativeItemOptions.map((item) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center px-3 py-2.5 sm:px-4 sm:py-3 rounded-lg border-2 text-left transition-all duration-200 ${
                            isSelected 
                              ? 'border-emerald-500 bg-emerald-50 text-emerald-900 font-medium' 
                              : 'border-gray-200 bg-white text-gray-700 hover:border-emerald-300'
                          }`}
                        >
                          <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded border flex items-center justify-center mr-2.5 sm:mr-3 shrink-0 transition-colors ${
